    ...
    d) part=$OPTARG
       case "$part" in
        ...
        all) echo deploy all
             ...
             source hvault-deploy/hvault-stack-status.sh hvault-svc HVaultECSEndpoint VAULT_SERVICE_URL
             source hvault-deploy/hvault-stack-status.sh hvault-svc HVaultOption3RoleConsumer1 VAULT_CONSUMER_ROLE_ARN_1
             source hvault-deploy/hvault-stack-status.sh hvault-svc HVaultOption3RoleConsumer2 VAULT_CONSUMER_ROLE_ARN_2
             source hvault-deploy/hvault-stack-status.sh hvault-svc HVaultRootToken VAULT_SERVICE_TOKEN_SECRET_ARN
             source hvault-deploy/hvault_auth_deploy.sh
             #source hvault-deploy/hvault_secret_deploy.sh
             source hvault-deploy/hvault_config_deploy.sh
             ;;
